...
/*
 	...
 	:	클래스가 객체가 될 때 클래스의 멤버가 가져야될 기본 값이 있을 것이다.
 		이 때 멤버들의 기본값을 셋팅해주는 역할을 하는 것이 생성자 함수이다.
 		...
 		***
 		생성자 함수를 하나도 정의하지 않으면 자바 가상 머신이 자동으로 기본 생성자 함수를 만들어서 호출해준다.
 		이 때 생성자 함수 안에 있는 내용은 비어있다.
 		
 		만일  생성자 함수를 하나라도 만들면 자바 가상 머신은 기본 생성자 함수를 만들지 않는다.
 		 따라서 생성자 함수를 정의할거면 기본 생성자 함수를 내용이 없더라도 정의해 주는게 좋다. 
...
/*
 	...
 	참고]
 		수학에서 비교연산자를 사용하듯이 한 번에 두개를 비교할 수는 없습니다.
